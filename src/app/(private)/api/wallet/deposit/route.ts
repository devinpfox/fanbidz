...
export async function POST(req: Request) {
  ...
  // Build success URL with optional return URL
  const baseSuccessUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/wallet/success?session_id={CHECKOUT_SESSION_ID}`;
  const successUrl = returnUrl
    ? `${baseSuccessUrl}&returnUrl=${encodeURIComponent(returnUrl)}`
    : baseSuccessUrl;
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    cancel_url: returnUrl
      ? `${process.env.NEXT_PUBLIC_SITE_URL}${returnUrl}`
      : `${process.env.NEXT_PUBLIC_SITE_URL}/wallet`,
    line_items: [
      {
        price_data: {
          currency: "usd",
